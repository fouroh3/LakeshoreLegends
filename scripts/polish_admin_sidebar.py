...
path = Path('src/pages/admin/AdminPage.tsx')
s = path.read_text()
...
start = s.index('function SectionButton({')
end = s.index('\nfunction AdminPanel({', start)
new_nav_components = r'''type NavTone = "cyan" | "sky" | "violet" | "amber" | "emerald";
...
'''
s = s[:start] + new_nav_components + s[end:]

old_bg = '    <div className="min-h-screen bg-[radial-gradient(circle_at_top,rgba(34,211,238,0.10),transparent_42%),#070707] px-3 py-5 text-zinc-100 sm:px-5">'
new_bg = '    <div className="min-h-screen bg-[radial-gradient(circle_at_top_left,rgba(34,211,238,0.12),transparent_34%),radial-gradient(circle_at_82%_8%,rgba(139,92,246,0.08),transparent_28%),#070707] px-3 py-5 text-zinc-100 sm:px-5">'
if old_bg not in s:
    raise SystemExit('admin background marker not found')
s = s.replace(old_bg, new_bg, 1)

nav_start_marker = '        <div className="grid gap-5 lg:grid-cols-[270px_minmax(0,1fr)]">\n          <aside'
nav_start = s.index(nav_start_marker)
main_marker = '\n\n          <main className="min-w-0">'
nav_end = s.index(main_marker, nav_start)

new_sidebar = r'''        <div className="grid gap-5 lg:grid-cols-[292px_minmax(0,1fr)] xl:gap-6">
...
s = s[:nav_start] + new_sidebar + s[nav_end:]
path.write_text(s)
