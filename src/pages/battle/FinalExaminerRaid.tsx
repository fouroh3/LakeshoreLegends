      ...
      <main className={teacher ? "mx-auto w-full max-w-[1500px] px-4 py-5" : "mx-auto flex min-h-[calc(100dvh-84px)] w-full max-w-[1900px] flex-col px-4 py-4"}>
        {teacher ? (
          <>
            ...
            {raid?.active ? <>
              ...
              <section className="mt-5 grid gap-4 xl:grid-cols-[0.85fr_1.5fr]">
                <div className="rounded-[24px] border border-cyan-300/15 bg-cyan-950/10 p-4"><div className="text-[10px] font-black tracking-[0.18em] text-cyan-200/80">LIVE CLASS HP</div><div className="mt-3 space-y-2">{raid.classes.map((unit) => { const maxHP = classMaxHP(unit.currentHP, unit.startingHP); return <div key={unit.classKey} className="rounded-xl border border-cyan-300/15 bg-black/20 p-3"><div className="flex justify-between gap-3 text-sm font-black"><span>{unit.label}</span><span className="text-cyan-100">{num(unit.currentHP)} / {num(maxHP)}</span></div><div className="mt-2"><Bar current={unit.currentHP} max={maxHP} className="from-cyan-400 via-sky-300 to-teal-200" /></div></div>; })}</div></div>
          ...
          {raid?.active ? <section className="mt-4 grid flex-1 gap-4 xl:grid-cols-[0.76fr_1.84fr]">
            <div className="rounded-[22px] border border-cyan-300/15 bg-[linear-gradient(145deg,rgba(8,24,34,0.88),rgba(8,12,20,0.96))] p-4"><div className="flex items-center justify-between"><div><div className="text-[10px] font-black tracking-[0.19em] text-cyan-200/85">RAID PARTIES</div><h2 className="mt-0.5 text-lg font-black">Class Vitality</h2></div><div className="rounded-full border border-cyan-300/15 bg-cyan-300/10 px-2.5 py-1 text-[9px] font-black tracking-[0.13em] text-cyan-100">LIVE HP</div></div><div className="mt-3 grid gap-2">{raid.classes.map((unit, index) => { const maxHP = classMaxHP(unit.currentHP, unit.startingHP); return <article key={unit.classKey} className="relative overflow-hidden rounded-xl border border-cyan-300/15 bg-black/20 px-4 py-3"><div className="absolute inset-y-0 left-0 w-1 bg-cyan-300/75" /><div className="flex items-center justify-between gap-3 pl-1"><div><div className="text-[9px] font-bold tracking-[0.14em] text-cyan-200/65">RAID UNIT 0{index + 1}</div><div className="mt-0.5 text-base font-black">{unit.label}</div></div><div className="text-right"><div className="text-2xl font-black text-cyan-100">{num(unit.currentHP)}</div><div className="text-[9px] font-bold tracking-[0.1em] text-zinc-500">OF {num(maxHP)}</div></div></div><div className="mt-2"><Bar current={unit.currentHP} max={maxHP} className="from-cyan-400 via-sky-300 to-teal-200" /></div></article>; })}</div></div>
