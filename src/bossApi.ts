...
function normalizeBossState(raw: any, fallback: { bossInstanceId: string; bossKey: string }): BossResult {
...
export async function submitBossDelta(args: {
  ...
  const res = await fetch(BASE_URL, {
    ...
    body: JSON.stringify({
      ...
      delta: args.delta,
      source: args.source ?? "",
      requestId: args.requestId ?? `${Date.now()}-${Math.random().toString(36).slice(2)}`,
    }),
  });
